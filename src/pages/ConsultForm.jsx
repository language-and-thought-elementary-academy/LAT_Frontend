      ...
      <div style={{ maxWidth: '1240px', margin: '0 auto', padding: '0 24px' }}>
        {success && (
          <div role="status" style={{ maxWidth: '760px', background: '#EFF8FF', border: '1px solid #B2DDFF', borderRadius: '14px', padding: '44px 36px' }}>
            <p style={{ font: "600 11px/1 'Noto Sans KR',sans-serif", letterSpacing: '.16em', color: '#1849A9', margin: '0 0 20px' }}>COMPLETED</p>
            <h2 style={{ font: "600 clamp(24px,2.4vw,32px)/1.4 'Noto Serif KR',serif", color: '#1849A9', margin: '0 0 18px' }}>{spec.successTitle}</h2>
            <p style={{ font: "400 16px/1.8 'Noto Sans KR',sans-serif", color: '#667085', margin: '0 0 32px' }}>{spec.successDesc}</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
              <SiteLink href="#/" className="hover-bg-1849a9" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minHeight: '50px', padding: '0 26px', background: '#155EEF', border: '1px solid #155EEF', borderRadius: '4px', color: '#fff', font: "600 15px/1 'Noto Sans KR',sans-serif", textDecoration: 'none' }}>홈으로</SiteLink>
              <SiteLink href="#/schedule" className="hover-bg-1849a9-fg-fff" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minHeight: '50px', padding: '0 26px', border: '1px solid #1849A9', borderRadius: '4px', color: '#1849A9', font: "600 15px/1 'Noto Sans KR',sans-serif", textDecoration: 'none' }}>시간표 보기</SiteLink>
            </div>
          </div>
        )}
          ...
          <form onSubmit={e => submitForm(spec, e)} noValidate style={{ maxWidth: '760px', display: 'flex', flexDirection: 'column', gap: '34px' }}>
            {fe.list.length > 0 && (
              <div role="alert" style={{ background: '#FDF2F2', border: '1px solid #E9C4C4', borderRadius: '10px', padding: '20px 22px' }}>
                <p style={{ font: "600 15px/1.5 'Noto Sans KR',sans-serif", color: '#9B2C2C', margin: '0 0 10px' }}>입력을 확인해주세요.</p>
                <ul style={{ margin: 0, paddingLeft: '18px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {fe.list.map((t, i) => (
                    <li key={i} style={{ font: "400 14px/1.6 'Noto Sans KR',sans-serif", color: '#9B2C2C' }}>{t}</li>
                  ))}
                </ul>
              </div>
            )}
            ...
            {spec.groups.map(g => (
              <fieldset key={g.key} style={{ border: 'none', margin: 0, padding: 0 }}>
                <legend style={{ font: "600 15px/1.5 'Noto Sans KR',sans-serif", color: '#1849A9', padding: 0, margin: '0 0 14px' }}>{g.label}{g.req ? ' *' : ''}</legend>
            ...
            {spec.fields.map(f => {
              ...
              return (
                <div key={f.key} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <label htmlFor={id} style={{ font: "600 15px/1.5 'Noto Sans KR',sans-serif", color: '#1849A9' }}>{f.label}{f.req ? ' *' : ''}</label>
                  {f.textarea ? (
                  ...
                  {f.hint && <p style={{ font: "400 13px/1.6 'Noto Sans KR',sans-serif", color: '#667085', margin: 0 }}>{f.hint}</p>}
                </div>
              )
            })}
            ...
            <label style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', padding: '20px 22px', background: '#EFF8FF', border: '1px solid #B2DDFF', borderRadius: '10px', cursor: 'pointer' }}>
              ...
              <span style={{ font: "400 14px/1.7 'Noto Sans KR',sans-serif", color: '#101828' }}>개인정보 수집 및 이용에 동의합니다. (필수)<br /><span style={{ color: '#667085' }}>수집 항목: 학생 이름, 학년, 보호자 연락처, 문의 내용 · 이용 목적: 상담 및 수업 안내 · 보유 기간: 상담 종료 후 1년</span></span>
            </label>

            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center' }}>
              <button type="submit" disabled={submitting} style={submitStyle(submitting)}>{submitting ? '전송 중…' : spec.submitLabel}</button>
              <a href="tel:0225670548" style={{ font: "500 14px/1.6 'Noto Sans KR',sans-serif", color: '#667085', textDecoration: 'none' }}>전화 상담 02-567-0548</a>
            </div>
          </form>
        )}
      </div>
