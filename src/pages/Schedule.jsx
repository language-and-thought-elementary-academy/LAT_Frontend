  ...
  return (
    <section style={{ background: '#fff', padding: 'clamp(56px,7vw,104px) 0' }}>
      <div style={{ maxWidth: '1240px', margin: '0 auto', padding: '0 24px' }}>
        ...
        {sEmpty && (
          <p style={{ font: "400 16px/1.8 'Noto Sans KR',sans-serif", color: '#667085', background: '#EFF8FF', border: '1px solid #B2DDFF', borderRadius: '12px', padding: '34px 30px', margin: 0 }}>조건에 맞는 개설반이 없습니다.<br />필터를 변경하거나 수강상담을 통해 개설 예정 수업을 문의해주세요.</p>
        )}

        <div style={{ marginTop: '60px', paddingTop: '44px', borderTop: '1px solid #E4EDF7' }}>
          <h2 style={{ font: "600 12px/1 'Noto Sans KR',sans-serif", letterSpacing: '.16em', color: '#1849A9', margin: '0 0 26px' }}>모집 상태 안내</h2>
          <dl style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(230px,1fr))', gap: '24px', margin: 0 }}>
            {statusLegend.map(s => (
              <div key={s.label}>
                ...
                <dd style={{ margin: 0, font: "400 15px/1.7 'Noto Sans KR',sans-serif", color: '#667085' }}>{s.desc}</dd>
              </div>
            ))}
          </dl>
          <div style={{ marginTop: '36px', display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
            ...
            <SiteLink href="#/classes" className="hover-bg-1849a9-fg-fff" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minHeight: '50px', padding: '0 26px', border: '1px solid #1849A9', borderRadius: '4px', color: '#1849A9', font: "600 15px/1 'Noto Sans KR',sans-serif", textDecoration: 'none' }}>수업 안내</SiteLink>
          </div>
        </div>
      </div>
    </section>
