  ...
  return (
    <article style={{ background: '#fff', border: '1px solid #D0D5DD', borderRadius: '14px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '24px 22px 26px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        <Heading style={{ font: "600 19px/1.3 'Noto Sans KR',sans-serif", color: '#1849A9', margin: 0 }}>{t.name}</Heading>
        <p style={{ font: "400 14px/1.6 'Noto Sans KR',sans-serif", color: '#667085', margin: 0 }}>담당 {t.grade}</p>
        <p style={{ font: "400 14px/1.6 'Noto Sans KR',sans-serif", color: '#667085', margin: '0 0 10px' }}>{t.program}</p>
        <SiteLink href={t.href} style={{ font: "600 14px/1 'Noto Sans KR',sans-serif", color: '#1849A9', textDecoration: 'none', borderBottom: '1px solid #155EEF', paddingBottom: '4px', alignSelf: 'flex-start', ...(linkMinHeight ? { minHeight: '24px' } : {}) }}>자세히 보기</SiteLink>
      </div>
    </article>
  )
}
